...
import { Send, Sparkles, Loader2, MessageSquare, Calendar as CalIcon } from 'lucide-react';
  ...
  const send = async (text) => {
    ...
    try {
      const resp = await fetch(`${API}/assistant/chat`, {
        ...
        body: JSON.stringify({
          session_id: sessionId, creator_id: DEMO_CREATOR_ID,
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_300px] gap-6">
      <div className="card-surface flex flex-col" style={{ height: '640px' }}>
        ...
        <div className="p-4 border-t border-white/5">
          <div className="flex gap-2">
            ...
            />
            <button onClick={() => send()} disabled={streaming || !input.trim()} data-testid="studio-send-btn"
      ...
      <div className="space-y-3">
        ...
        {SUGGESTIONS.map((s, i) => (
          <button key={i} onClick={() => send(s)} disabled={streaming} data-testid={`studio-suggest-${i}`}
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} data-testid="studio-page">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-zinc-400 max-w-xl">Grounded in your DNA. Chat, plan, ship.</p>
        </div>
        <div className="inline-flex rounded-full border border-white/10 p-1 bg-white/[0.02]">
          <button
            onClick={() => setTab('chat')}
            data-testid="tab-chat"
            className={`px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 ${tab === 'chat' ? 'bg-violet-500/20 text-white' : 'text-zinc-400'}`}
          ><MessageSquare size={14} /> Assistant</button>
          <button
            onClick={() => setTab('plan')}
            data-testid="tab-plan"
            className={`px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 ${tab === 'plan' ? 'bg-violet-500/20 text-white' : 'text-zinc-400'}`}
          ><CalIcon size={14} /> Weekly plan</button>
        </div>
      </div>

      {tab === 'chat' ? <AssistantChat /> : <WeeklyPlan />}
    </motion.div>
  );
}
