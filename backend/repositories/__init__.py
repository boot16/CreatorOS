...
class PlatformRepo:
    ...
    async def mark_disconnected(self, cp_id: str):
        await self.db.connected_platforms.update_one(
            {"id": cp_id},
            {"$set": {"connection_status": "disconnected", "updated_at": _now()}},
        )
...
class CredentialRepo:
    ...
    async def delete(self, connected_platform_id: str):
        await self.db.platform_credentials.delete_one({"connected_platform_id": connected_platform_id})
